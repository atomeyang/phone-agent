...
[helpers/docs/Gemma4-E2B-D8400-Hybrid-Deployment.docx](helpers/docs/Gemma4-E2B-D8400-Hybrid-Deployment.docx) 是完整的复现手册（环境、架构决策、流水线、陷阱目录、验证数据、代码摘录、诊断方法；目前为英文）。它由 `helpers/tools/make_reproduction_doc.py` 生成，任何一次重跑之后都可以重新出文档。

部署形态的只读参考：`/mnt/d/codes/Gallery/gitlab/phonevlm/mtk` 下的 MT6899 MiniCPM-V 4.6 交付（W8A16 视觉 DLA + MNN Q4 文本，同一代 SoC、同一个 NeuroPilot 9.0.9 SDK）。
...
| 记忆 | `runtime/agent/agent_memory.cpp` | 有界磁盘存储（事实、偏好、笔记、episode 摘要），带 BM25 + 中文 bigram + 时间/重要性衰减、近似重复合并、剪枝，以及**按槽位更新**："记住我叫李雷" 之后 "改成记住我叫杨雷" 会替换该记录，而不是两条并存 |
...
`helpers/tools/agent_live_probe.cpp`（x86 上跑真实权重）测过这段块能放在哪里：以 `<|tool_response>` *开头*的用户轮次会让模型立刻结束本轮；把它放进模型轮次、紧跟 MNN 的 `<turn|>`\n`<|turn>model`\n 之前，会让模型重新吐出结束 token 然后停下。先用一行纯文本引出观测、再追加原生块，既能得到回答、又保持 MNN 的 prompt-cache 前缀有效，运行时采用的就是这种组合。
...
* 工具*名字*会漂移（`get_time`、`current_time`、`calc`、单字母笔误）；注册表在报错之前会先解析别名和单字符偏差。
* 一条确定性的路由提示（算术、时间、图像、"记住"、"召回"）追加在用户消息上，绝不放进系统提示词——系统提示词必须逐字节一致，prompt cache 才能命中。
...
| 图像轮次（修复后） | dog 测试图 + "describe this image in detail" → "I can see a dog in the image." |
...
每个轮次先做分类（`runtime/agent/agent_intent.cpp` 中的原生模式匹配，不调用模型），得到的策略决定后续一切：是否查询长期记忆、是否预期工具调用、答案是否必须保持纯文本。问候语不触碰记忆库；算术或时钟问题由路由直接给出确切的调用；记忆类问题直接由存储回答、不调工具；图像描述禁止工具（`image_info{data:…` 那类尝试正是这样被拦下的）。判定结果以 `intent` 事件发出，并在聊天里显示成一张卡片。对图像描述，如果答案拒绝作答、把问题反弹回来或过短，会回退到出货的单轮 prompt（"Describe this image."）——真机实测："This is a photograph of a dog outdoors. The dog appears to be a light-colored … medium-sized dog with a relaxed expression …"。

### 附件是一次性的

附件只作用于随它一起发出的那条消息：本轮发出后，芯片回到 未附加图片，清屏 也会把它卸下，另外有 移除图片 按钮可按需解除。关于这张图的后续提问仍然能通过会话记录解析（图像轮次保留了自己的缓存画布）。
...
图像轮次现在每一步都走同一条路径：运行时每次调用都把图像描述交给 driver，driver 只在自身 KV 状态是新 prompt 前缀时才走增量 prefill，否则重跑视觉塔并做全量 prefill（此前的 bug 是图像轮次的第 2 步——工具调用或重试之后——走了增量路径，报 "an image turn must use a full prefill" 失败）。图像提示要求纯文本描述，并刻意不提 `image_info`，因为否则 Q4 导出会试图把图像字节交给那个工具；现在无法解析的工具形态答案会触发一次纯文本重试。

真机实测：dog 测试图 + "please describe this image in detail" 答出 "I can see a dog in the image."。开放式建议（"how to make this image better"）仍然超出这份导出的能力——它会产出破碎的工具片段或两个无关 token，所以 UI 为这条真正验证过的路径提供了「描述图片」快捷操作。
...
主机套件覆盖 JSON 编解码、calculator、每个工具、记忆库、会话/记录存储、规划器、协议解析、完整轮次循环（工具错误、预算、强制作答）、图像轮次、压缩、会话、事件契约，以及面向 App 的文件协议（请求进、事件出、响应文档、取消哨兵、旧版 benchmark 触发、畸形输入）。实时探针用设备上同样的 MNN 调用驱动导出的权重：连续三轮共用一个引擎、首轮之后零重载（`full_prefills=0 delta_generations=5 engine_resets=0`），calculator 往返完成，反复 `reset()` + 重新 prefill 产出完全一致的结果。

## 记忆系统

三层，各有自己的生命周期：会话记录（说过的一切，随会话删除）、每会话的滚动摘要，以及跨会话的存储。只有最后一层才是 UI 意义上的*记忆*——`runtime/agent/agent_memory.{h,cpp}`，纯文件存储，因此数据能跨 App 更新存活，也能直接用 `cat` 查看：
...
| `importance`、`pinned` | 检索与剪枝使用的 0–1 权重；pinned 记录永不被剪枝，且总是注入 |
| `created_ms`、`updated_ms`、`last_access_ms`、`access_count` | 衰减所依据的时间与频率信号 |
| `session`、`turn`、`tags` | 来源信息，一条记录可以追溯到写入它的那一轮 |

**写入。** `AgentRuntime::write_memories()` 在每一轮之后运行。规则抽取器（`agent_prompt.cpp` 里的 `extract_memories()`）把显式的"记住…" / "remember …" 变成 `fact`（重要性 0.9），把身份陈述（"我叫…"、"我住在…"、"call me …"）变成 `fact`（0.8），把偏好（"我喜欢…"、"i prefer …"）变成 `preference`（0.7）；祈使前缀本身会被剥掉，所以库里存的是"我叫杨雷"而不是"记住: 我叫杨雷"。纠正标记（"改成"、"改为"、"instead"）会把文本截到纠正后的部分并把该记录标记为纠正，因此它替换旧值而不是新增一条。其余内容会被记为 `episode`（"Q: … -> A: …"，重要性 0.3），除非打开 `store_episodes`，否则存储会丢弃它——轮次 episode 属于会话记录，通过 `search_history` 检索。每次写入都发出 `memory_write`，带 id、kind 以及该记录是 `new`、`merged` 还是 `updated`。`GEMMA4_AGENT_MODEL_MEMORY=1` 会加一次小的模型调用，用其结果覆盖规则抽取（默认关闭：出货版 Q4 导出是在规则抽取下验证的）。

**去重。** `MemoryStore::add()` 按顺序尝试三条规则，命中即停：

1. **槽位冲突** —— `extract_slot()` 把前缀映射到键（`name`、`city`、`job`、`preference`）；同一 kind、同一键下的不同值会替换旧记录，所以"记住我叫李雷"之后接"改成记住我叫杨雷"只留一条记录；
2. **显式纠正** —— 标记了 `correction` 的记录，只要与同 kind 中最相似的记录 token Jaccard 相似度达到 0.2 就替换它，即使措辞变化很大；
3. **近似重复** —— Jaccard ≥ 0.72 时合并进已有记录（文本取更长者、tag 取并集、重要性取更高、访问计数 +1）。

**检索。** `MemoryStore::search()` 是一次本地 BM25（k1 = 1.2，b = 0.6），词元来自 `tokenize()`：ASCII 单词统一小写，CJK 则每个码点加每个 bigram，所以"手机"也能匹配"我手机壳是紫色的"。词法得分再依次乘以 kind 权重（episode 0.55、fact/preference 1.0、note 0.9）、`0.6 + 0.8 * importance`、时间因子 `0.7 + 0.3 * 0.5^(age / half_life)`（半衰期 14 天），再加上 `1 + 0.05 * min(access_count, 8)` 的频率加成、精确短语匹配的 1.5 加成，以及 pinned 时的 `1 + 0.5 * importance`。低于 `memory_min_score`（0.12）的一律丢弃，所以一句问候完全不会召回任何东西。

`AgentRuntime::recall()` 在其上加了一层策略：`"memory": false` 时什么都不返回；命中记录若来自*当前*会话则分数乘 1.6（本轮对话刚建立的事实必须压过旧会话对同一问题的答案）；pinned 记录无条件追加；结果列表上限为 `recall_top_k`（4）+ 4。这段内容会前置到当前用户消息里——绝不单独成为一条消息，因为插在 prompt 中间的新消息会让 KV 前缀失效：
...
这段措辞是刻意的：真机上它曾用*记住的*最爱颜色去回答"它是什么颜色的？"（问的是一张图），因为被召回的事实看起来像是在作答。
...
一条轮次记录保存的是*模型当时看到的原始 `messages`*——用户、工具调用、观测结果、答案——而不只是用户可见的问答。逐字重放这些内容能让 prompt 成为上一轮的严格前缀延伸，这正是 MNN 的 prompt cache 只预填充新增后缀的前提；因此记录是 append-only 的（`append_turn`），只有当一个图像轮次必须用它的文字说明替换软 token 时才会重写。每条记录还带上该轮的耗时与用量（`ttft_ms`、`prefill_ms`、`npu_ms`、`decode_tps`、`prompt_tokens`），App 上显示的数字就来自这里。

**进入 prompt 的是什么。** `render_history()` 依次组装系统提示词、滚动摘要（如果存在）、`summary_upto` 之后的每一轮，以及本轮自己的往返。一张图*只在承载它的那一轮*贡献 NPU 软 token：更早的图像轮次渲染时，`<|image>…<image|>` 会被替换成 `[image: <note>]`，其中 note 是模型当时对这张图的描述再加 `; described as: …`，而且这个替换会被持久化。于是后续 prompt 携带的是一段文字描述而不是 130 个软 token，也永远不会重跑视觉塔（`reuse_history_images` 可以有意把软 token 钉回来）。保留轮次里的工具调用与观测保持原样，只有压缩会折叠它们。
...
**预算。** `history_budget_tokens()` 从 prompt 预算推导记录预算：`context_budget_tokens`（1700）减去实测的系统提示词及其工具声明（677），再减去一轮自身的 420 token 额度，下限 128——也就是上表和 helpers/STATUS.md 里的 603。在 `agent-config.json` 中设置 `history_budget_tokens` 改为钉死一个显式值。`"history": false` 切换到全新上下文模式：只有系统提示词、当前消息和本轮的往返。

**会话笔记（Notetaker）。** `update_session_notes()` 为每个"留下了东西"的轮次追加一条有界 bullet——`[image t7] <这张图看到了什么>`、`[memory t7] <存了什么>`、`[tools t7] calculator, now -> <答案开头>`——只保留最后 `session_notes_lines`（8）行、`session_notes_chars`（900）个字符，存在会话 meta 里，因此重启后仍在。它被注入到*当前*用户消息里（`[SESSION NOTES] earlier in this conversation: …`）而不是放在历史之前，并且会在运行时可自行作答的轮次（算术、时钟、单位换算、字数统计）上被抑制：评测 A/B 发现，带着笔记时某个算术轮次什么都没答出来，去掉之后答对了 `42+16 = 58`。它也是让旧图在软 token 离开窗口后仍然可用的原因。
...
向 Mobile-Agent 系列借用的两级窗口正是上述形态的来源，但出货运行时用"一张图只活在它自己的轮次"加压缩来实现它：`image_window_turns`、`tool_detail_window_turns`、`recent_turns_max` 和 `max_context_turns` 仍会被解析并由 `capabilities` 上报，但运行时不再读取它们——真正驱动策略的只有 `recent_turns_min`、`compaction_min_turns` 以及笔记的几条上限。
...
`env.sh` 从 `GEMMA4_GRID_W/GEMMA4_GRID_H` 推导出 `GEMMA4_CANVAS_W/H`、软 token 预算和输出目录（`out-<W>x<H>/`）；`out/mnn/` 下的文本主干因为与画布无关而被共用。APK 通过 `-PgridColumns/-PgridRows` 拿到同样的数字，并在头部打印出来（`480x624 fit + padding, W8A16 vision`），所以编译进 DLA 的几何与 App 写入的 letterbox 是否一致一眼可见——runner 会拒绝头部与自己几何不符的画布。

更小的画布并非没有代价：横版输入（文档、二维码）损失最多 token，模型可能"看不见"它们。对比的测量数据与 PC 侧实现在 [helpers/STATUS.md](helpers/STATUS.md)。
...
`helpers/scripts/12_reproduce_all.sh` 默认使用出货的 30x39 profile（`out-30x39/`），启动前检查每一项前置条件，依次跑步骤 1–12（主机 agent 测试套件、arm64 runner、APK 以及 x86 实时 agent 探针）外加手册重建，最后做一致性检查（DLA/TFLite/MNN/APK 都在、组装后的权重与导出逐字节一致——trap T16）。用 `--grid 54 45` 回到最初的 profile，`--from N` 续跑，`--no-device` 在 APK 之后停下，`--force` 忽略已有产物。如果 `$OUT` 里已经有另一个画布的图，它会拒绝启动，因为步骤 01 会默默复用它。所有输出都会被 tee 进 `logs/reproduce-<timestamp>.log`。

重建 APK 时可能在 `:app:packageDebug` 撞上 AGP 的 `integer overflow`：增量打包器的尺寸计算是 32 位的，而 APK 有 2.9 GB。步骤 8 会识别这条信息并在 `:app:clean` 之后重试（非增量打包），所以重跑不需要手工清理。

`helpers/scripts/00_clean_generated.sh` 只删除流水线生成的东西（`out*/`、`logs/`、`work/`、Gradle 构建目录、`__pycache__`）；文档、工具和所有输入都不动。它先打印带体积的计划，除非给出 `--yes` 否则会先询问；`--device` 还会清理手机上的内容。
...
| 6 | `helpers/tools/run_agent_host_tests.sh` | 在主机上构建并运行 256 项检查的原生 agent 套件（`work/agent-tests/`） |
...
demo 本体是 `android/` 加 `runtime/`（App 驱动的那个 arm64 进程）以及 `helpers/scripts/`——构建、组装、部署并运行它的那些步骤。只负责准备输入或做测量的内容都放在 `helpers/` 下。
...
那半边的细节见 [helpers/README.md](helpers/README.md)。生成产物无论由哪个步骤产生，都留在根目录（`out/`、`out-<W>x<H>/`、`logs/`、`work/`）。
