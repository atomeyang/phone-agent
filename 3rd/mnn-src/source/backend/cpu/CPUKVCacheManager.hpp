...
static inline const CoreFunctions* CPUAttentionCoreFunctions(Backend* backend) {
    const auto core = static_cast<CPUBackend*>(backend)->functions();
    if (core->matmulBytes > 0 && core->matmulBytes != core->bytes) {
        return MNNGetCoreFunctions();
    }
    return core;
}
