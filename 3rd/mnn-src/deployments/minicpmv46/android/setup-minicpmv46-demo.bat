...
set "PACKAGE=com.example.minicpmv46.neuropilot"
set "ACTIVITY=com.example.minicpmv46.neuropilot/.MainActivity"
set "REMOTE_ROOT=/data/local/tmp/minicpmv46-neuropilot"
set "EXPECTED_MIB=3551"
set "SCRIPT_DIR=%~dp0"
...
call :find_apk
if not defined APK (
    echo ERROR: PhoneVLM Adaptive APK was not found beside this script. 1>&2
    exit /b 1
)

set "STATE_FILE=%TEMP%\minicpmv46-demo-%RANDOM%-%RANDOM%.tmp"
...
"%ADB%" shell "for pid in $(pidof minicpmv46-hybrid-runner 2>/dev/null); do kill -9 "$pid" 2>/dev/null || true; done; for pid in $(pidof sh 2>/dev/null); do script=$(tr '\0' '\n' < /proc/$pid/cmdline 2>/dev/null ^| sed -n '2p'); case "$script" in ./minicpmv46-neuropilot-bridge.sh^|*/minicpmv46-neuropilot-bridge.sh) kill -9 "$pid" 2>/dev/null || true ;; esac; done; for file in '%REMOTE_ROOT%/bridge.pid' '%REMOTE_ROOT%/app-runner.pid'; do if [ -f "$file" ]; then pid=$(cat "$file" 2>/dev/null); [ -n "$pid" ] && kill -9 "$pid" 2>/dev/null || true; rm -f "$file"; fi; done; rm -f '%REMOTE_ROOT%/app-service-ready' '%REMOTE_ROOT%/app-service-start' '%REMOTE_ROOT%/minicpmv46-neuropilot-bridge.sh' '%REMOTE_ROOT%/minicpmv46-hybrid-runner' '%REMOTE_ROOT%/models/visual_560x392_fp16_mt6899.dla' '%REMOTE_ROOT%/models/visual_392x504_fp16_mt6899.dla' '%REMOTE_ROOT%/models/visual_504x392_fp16_mt6899.dla'" >nul
"%ADB%" shell am force-stop "%PACKAGE%" >nul
"%ADB%" shell am start -S -W -f 0x10008000 -n "%ACTIVITY%" >nul || goto :adb_error

echo Extracting packaged models and loading the resident runtime...
set /a BRIDGE_STARTED=0
for /l %%I in (1,1,1200) do (
    if !BRIDGE_STARTED! EQU 0 (
        "%ADB%" shell "test -f '%REMOTE_ROOT%/minicpmv46-neuropilot-bridge.sh' && test -f '%REMOTE_ROOT%/app-service-start'" >nul 2>&1
        if not errorlevel 1 (
            "%ADB%" shell "chmod 755 '%REMOTE_ROOT%/minicpmv46-neuropilot-bridge.sh'; cd '%REMOTE_ROOT%'; setsid sh ./minicpmv46-neuropilot-bridge.sh > bridge-daemon.log 2>&1 < /dev/null & echo $! > bridge.pid" >nul
            set /a BRIDGE_STARTED=1
            echo Packaged runtime extracted; loading NPU and MNN models...
        )
    )
...
echo PhoneVLM Adaptive is installed and open.
...
:find_apk
set "APK="
if exist "%SCRIPT_DIR%PhoneVLM-Adaptive-v1.3.1.apk" set "APK=%SCRIPT_DIR%PhoneVLM-Adaptive-v1.3.1.apk"
if not defined APK if exist "%SCRIPT_DIR%MiniCPM-V-4.6-NPU-MNN-v1.3.0.apk" set "APK=%SCRIPT_DIR%MiniCPM-V-4.6-NPU-MNN-v1.3.0.apk"
if not defined APK if exist "%SCRIPT_DIR%app-debug.apk" set "APK=%SCRIPT_DIR%app-debug.apk"
if not defined APK if exist "%SCRIPT_DIR%app\build\outputs\apk\debug\app-debug.apk" set "APK=%SCRIPT_DIR%app\build\outputs\apk\debug\app-debug.apk"
exit /b 0
