...
REMOTE_ROOT="/data/local/tmp/minicpmv46-neuropilot"
EXPECTED_MIB=3551
...
APK=""
for candidate in \
    "$SCRIPT_DIR/PhoneVLM-Adaptive-v1.3.1.apk" \
    "$SCRIPT_DIR/MiniCPM-V-4.6-NPU-MNN-v1.3.0.apk" \
    "$SCRIPT_DIR/app-debug.apk" \
    "$SCRIPT_DIR/app/build/outputs/apk/debug/app-debug.apk"; do
    if [[ -f "$candidate" ]]; then
        APK="$candidate"
        break
    fi
done
[[ -n "$APK" ]] || fail "The PhoneVLM Adaptive APK was not found beside this script."
ADB_APK="$APK"
...
"$ADB" shell "
    for pid in \$(pidof minicpmv46-hybrid-runner 2>/dev/null); do
        kill -9 \"\$pid\" 2>/dev/null || true
    done
    for pid in \$(pidof sh 2>/dev/null); do
        script=\$(tr '\\0' '\\n' < \"/proc/\$pid/cmdline\" 2>/dev/null | sed -n '2p')
        case \"\$script\" in
            ./minicpmv46-neuropilot-bridge.sh|*/minicpmv46-neuropilot-bridge.sh)
                kill -9 \"\$pid\" 2>/dev/null || true ;;
        esac
    done
    ...
    rm -f '$REMOTE_ROOT/app-service-ready' \
        ...
        '$REMOTE_ROOT/minicpmv46-neuropilot-bridge.sh' \
        '$REMOTE_ROOT/minicpmv46-hybrid-runner' \
        '$REMOTE_ROOT/models/visual_560x392_fp16_mt6899.dla' \
        '$REMOTE_ROOT/models/visual_392x504_fp16_mt6899.dla' \
...
printf 'Extracting packaged models and loading the resident runtime...\n'
deadline=$((SECONDS + 1200))
next_report=0
bridge_started=0
while (( SECONDS < deadline )); do
    if (( bridge_started == 0 )) && \
            "$ADB" shell "test -f '$REMOTE_ROOT/minicpmv46-neuropilot-bridge.sh' \
                && test -f '$REMOTE_ROOT/app-service-start'" >/dev/null 2>&1; then
        "$ADB" shell "
            chmod 755 '$REMOTE_ROOT/minicpmv46-neuropilot-bridge.sh'
            cd '$REMOTE_ROOT'
            setsid sh ./minicpmv46-neuropilot-bridge.sh \
                > bridge-daemon.log 2>&1 < /dev/null &
            echo \$! > bridge.pid
        " >/dev/null
        bridge_started=1
        printf 'Packaged runtime extracted; loading NPU and MNN models...\n'
    fi
    ...
    if "$ADB" shell "test -s '$REMOTE_ROOT/app-service-ready'" >/dev/null 2>&1; then
        ...
        printf 'Ready: %s\nPhoneVLM Adaptive is installed and open.\n' "$READY_MESSAGE"
        exit 0
    fi
