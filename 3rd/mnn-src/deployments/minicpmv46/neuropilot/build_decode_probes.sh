...
OUTPUT="${OUTPUT:-$ROOT/build/decode-probes}"
...
printf '[1/3] Exporting and validating decode probes\n'
"$EXPORT_PYTHON" "$ROOT/export_decode_probes.py" \
    --model "$MODEL" \
    --output-dir "$OUTPUT" \
    --capacity 256 \
    --past-length 86 \
...
for kind in linear_decode full_decode; do
    printf '[2/3] Converting %s to TFLite\n' "$kind"
    ...
    printf '[3/3] Compiling %s as pure MDLA for MT6899\n' "$kind"
...
printf 'Both decode layer types compiled as pure MDLA: %s\n' "$OUTPUT"
