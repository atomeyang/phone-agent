        ...
        try:
            with contextlib.redirect_stdout(log_fp):
                ...
                if self.mnnconvert is None:
                    ...
                    env = os.environ.copy()
                    # Use system libstdc++ instead of SDK's outdated version
                    sdk_root = env.get('NEURON_SDK_ROOT', '')
                    sdk_lib = os.path.join(sdk_root, 'linux-x86_64', 'lib')
                    if sdk_lib and os.path.exists(sdk_lib):
                        paths = [p for p in env.get('LD_LIBRARY_PATH', '').split(':') if p and p != sdk_lib]
