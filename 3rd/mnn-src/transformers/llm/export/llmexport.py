...
class LlmExporter(torch.nn.Module):
    ...
    def load_model(self, model_path):
        ...
        # Local patch (mtkd8400deploy): this deployment runs the vision tower on
        # the MTK NPU and only needs the text backbone from MNN.  Set
        # MNN_EXPORT_TEXT_ONLY=1 to skip the MNN vision/audio exports.
        text_only = os.environ.get('MNN_EXPORT_TEXT_ONLY') == '1'
        self.visual = None if text_only else self.model.visual
        self.audio = None if text_only else self.model.audio
