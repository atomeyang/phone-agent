...
void Llm::setTokenCallback(std::function<bool(int, int)> callback) {
    mContext->token_callback = std::move(callback);
}

void Llm::setSoftTokenProvider(SoftTokenProvider provider) {
    mSoftTokenProvider = std::move(provider);
}

int Llm::softTokenId() const {
    // The placeholder id comes straight from llm_config.json (`image_pad`, for
    // Gemma 4 that is 258880 - the same token the runner's prompt template uses).
    return mConfig->config_.value("image_pad", 258880);
}

void Llm::spliceSoftTokens(VARP embeddings, const std::vector<int>& tokens,
                           int occurrence_offset) {
    if (mSoftTokenProvider == nullptr || embeddings == nullptr ||
        embeddings->getInfo() == nullptr) {
        return;
    }
    const int placeholder = softTokenId();
    const int hidden = mConfig->hidden_size();
    float* data = embeddings->writeMap<float>();
    if (data == nullptr) {
        return;
    }
    int occurrence = occurrence_offset;
    for (size_t index = 0; index < tokens.size(); ++index) {
        if (tokens[index] != placeholder) {
            continue;
        }
        const float* soft = mSoftTokenProvider(occurrence);
        if (soft != nullptr) {
            std::memcpy(data + index * hidden, soft, static_cast<size_t>(hidden) * sizeof(float));
        }
        ++occurrence;
    }
}
...
void Llm::initRuntime() {
    ...
    } else if (mConfig->precision() == "bf16") {
        cpuBackendConfig.precision = BackendConfig::Precision_Low_BF16;
    }
...
std::vector<int> Llm::generate(const std::vector<int>& input_ids, int max_tokens) {
    ...
    if(!passExecute) {
        if (0 == mBlockSize || input_ids.size() <= mBlockSize) {
            ...
            // Inject the multimodal soft tokens of every image placeholder this
            // chunk carries (the provider is supplied by the caller).
            spliceSoftTokens(hidden_states, input_ids, mSoftTokenOffset);
        ...
        for (int i = 0; i < loop_size; i++) {
            ...
            spliceSoftTokens(input_embeds, chunk_ids, mSoftTokenOffset);
            mSoftTokenOffset += static_cast<int>(std::count(chunk_ids.begin(), chunk_ids.end(),
                                                            softTokenId()));
...
void Llm::response(const ChatMessages& chat_prompts, std::ostream* os, const char* end_with, int max_new_tokens) {
    ...
    if (mConfig->prompt_cache() && !mCachedPromptText.empty()) {
        ...
        // The delta may carry the placeholders of a newly attached image; the
        // provider indexes occurrences over the whole prompt, so hand it the
        // number of placeholders that live in the cached prefix.
        mSoftTokenOffset = static_cast<int>(
            std::count(full_tokens.begin(), full_tokens.begin() + token_split, softTokenId()));
