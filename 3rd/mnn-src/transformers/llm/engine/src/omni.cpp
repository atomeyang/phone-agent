...
Omni::Omni(std::shared_ptr<LlmConfig> config) : Llm(config) {
    if (config->is_visual()) {
        ...
        mDisableDeepstack = config->config_.value("disable_deepstack", false);
        mDisableVisionEmbedding = config->config_.value("disable_vision_embedding", false);
    }
    if (config->is_audio()) {
...
bool Omni::load() {
    ...
    if (mConfig->mllm_config_.is_null()) {
        ...
        } else if (mConfig->precision(true) == "bf16") {
    ...
    for (int i = 0; i < input_ids.size(); i++) {
        ...
        if (inVision) {
            ...
            if (hasDeepStack) {
                ...
                if (mDisableDeepstack) {
                    auto shape = deepstack_embedding->getInfo()->dim;
                    deepstack_embedding = Express::_Fill(
                        _var<int>(shape, {static_cast<int>(shape.size())}), _Scalar<float>(0.0f));
                }
                deepstacks.push_back(deepstack_embedding);
            }
            auto mul_embedding = mVisionEmbeddings[vision_idx++];
            if (mDisableVisionEmbedding) {
                auto shape = mul_embedding->getInfo()->dim;
                mul_embedding = Express::_Fill(
                    _var<int>(shape, {static_cast<int>(shape.size())}), _Scalar<float>(0.0f));
            }
            embeddings.push_back(txt_embedding);
            embeddings.push_back(mul_embedding);
