...
struct LlmContext {
    ...
    std::function<bool(int, int)> token_callback;
    // llm status
...
public:
    ...
    void setTokenCallback(std::function<bool(int, int)> callback);
    // Multimodal splice hook: for every `<|image|>` (image_pad) token the engine
    // is about to prefill, the provider is asked for the soft-token row of that
    // occurrence (0-based, counted over the whole prompt).  Returning nullptr
    // keeps the token's own embedding.  This is what lets an image turn prefill
    // only its *delta* instead of re-creating the engine and re-running the
    // whole prompt (see STATUS.md trap T15 and the agent's MnnAgentLlm).
    using SoftTokenProvider = std::function<const float*(int occurrence_index)>;
    void setSoftTokenProvider(SoftTokenProvider provider);
...
private:
    SoftTokenProvider mSoftTokenProvider;
    // Number of image placeholders that appear before the chunk currently being
    // prefilled (set by the prompt-cache delta path, 0 for a full prefill).
    int mSoftTokenOffset = 0;
    void spliceSoftTokens(Express::VARP embeddings, const std::vector<int>& tokens,
                          int occurrence_offset);
    int softTokenId() const;
